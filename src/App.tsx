...
export function App() {
  ...
  // Move photo to a specific position number (1-based)
  const handleMovePhoto = (photoId: string, targetPosition: number) => {
    const fromIndex = photos.findIndex((p) => p.id === photoId);
    if (fromIndex === -1) return;
    const targetIndex = Math.max(0, Math.min(photos.length - 1, targetPosition - 1));
    ...
    // Reorder gridCells to align with the new photo order
    setGridCells((prevCells) => {
      const nextCells: GridCell[] = [];
      nextPhotos.forEach((p) => {
        const matching = prevCells.filter((c) => c.photoId === p.id);
        nextCells.push(...matching);
      });
      return nextCells;
    });
  };

  // Move a cell to a specific position number (1-based)
  const handleMoveCellToPosition = (cellId: string, targetPosition: number) => {
    setGridCells((prev) => {
      const fromIndex = prev.findIndex((c) => c.id === cellId);
  ...
  // Sort photos alphabetically A-Z
  const handleSortPhotosByName = () => {
  ...
  const handlePrint = () => {
    executePrint(photos, gridCells, config);
  };
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            <PageViewer
              ...
              onMoveCellToPosition={handleMoveCellToPosition}
              onUploadClick={() => hiddenFileInputRef.current?.click()}
            />
            ...
            <PhotoList
              ...
              onSortByName={handleSortPhotosByName}
              onReverseOrder={handleReversePhotos}
            />
          </div>
        </div>
      </div>
