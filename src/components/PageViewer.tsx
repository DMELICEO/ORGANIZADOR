...
interface PageViewerProps {
  ...
  onMoveCellToPosition?: (cellId: string, targetPosition: number) => void;
  onUploadClick: () => void;
}
                ...
                >
                  {inst ? (
                    ...
                    >
                      ...
                      {/* Position Badge in cell */}
                      <div className="absolute top-1 left-1 bg-slate-900/80 text-white text-[9px] font-mono font-bold px-1.5 py-0.5 rounded shadow-xs z-20 pointer-events-none">
                        #{gridCells.findIndex(c => c.id === inst.instanceKey.split('-')[0]) + 1}
                      </div>
                      ...
                      <div className="absolute top-1 right-1 flex flex-col gap-1 opacity-0 group-hover/cell:opacity-100 transition-opacity z-30 pointer-events-auto">
                        <div className="flex items-center gap-0.5 bg-slate-900/90 backdrop-blur-xs text-white p-0.5 rounded shadow">
                          ...
                          {/* Direct Position Number Selector */}
                          <div className="flex items-center gap-0.5 bg-white/20 px-1 py-0.5 rounded text-[10px]" title="Posición en la hoja - cambia para reordenar">
                            <span className="text-[9px] text-slate-300 font-mono">#</span>
                            <select
                              value={gridCells.findIndex(c => c.id === inst.instanceKey.split('-')[0]) + 1}
                              onChange={(e) => {
                                e.stopPropagation();
                                const currentId = inst.instanceKey.split('-')[0];
                                const targetPos = parseInt(e.target.value, 10);
                                if (!isNaN(targetPos) && onMoveCellToPosition) {
                                  onMoveCellToPosition(currentId, targetPos);
                                }
                              }}
                              onClick={(e) => e.stopPropagation()}
                              onMouseDown={(e) => e.stopPropagation()}
                              className="bg-white text-slate-900 font-bold rounded px-1 py-0.2 text-[10px] cursor-pointer focus:outline-none"
                            >
                              {gridCells.map((_, i) => (
